...
$consulta="select g.id_gestion, c.ruc, p.total
...
plan='NOCTURNO ESPECIAL' desc,
ob_gesj desc;"; 
$resultado=pg_query($consulta) or die (pg_last_error());

if(pg_num_rows($resultado)==0){
  pg_query("update tbl_gestion_cobranzas_cobros set gestion_juridica=date 'now()', gestion_final='".$conclusion."', 
...
{
  pg_query("update tbl_instalacion set estado_servicio='r' where id_instalacion=(select max(id_instalacion) from tbl_instalacion where id_cliente=(select id_cliente from tbl_gestion_cobranzas_cobros where id_gestion='".$id."'))") or die("Error de conexion. ". pg_last_error());
...
values
(select id_instalacion from tbl_factura_venta where id_factura_venta='".$id."'),
(select id_sucursal from tbl_instalacion where id_cliente=(select id_cliente from tbl_gestion_cobranzas_cobros where id_gestion='".$id."'),
(select (max(num_orden)::int)+1 from tbl_orden_trabajo where id_sucursal=(select id_sucursal from tbl_instalacion where id_cliente=(select id_cliente from tbl_gestion_cobranzas_cobros where id_gestion='".$id."')), 
date 'now()',
'4', 
...
{
  pg_query("update tbl_instalacion set estado_servicio='d' where id_instalacion=(select id_instalacion from tbl_factura_venta where id_factura_venta='".$id."')") or die("Error de conexion. ". pg_last_error());
}
if(!strcmp($conclusion, "PAGAN Y SE RETIRAN"))
{
  pg_query("update tbl_instalacion set estado_servicio='t' where id_instalacion=(select id_instalacion from tbl_factura_venta where id_factura_venta='".$id."')") or die("Error de conexion. ". pg_last_error());
}
if(!strcmp($conclusion, "EQUIPOS DEVUELTOS"))
{
  pg_query("update tbl_instalacion set estado_servicio='e' where id_instalacion=(select id_instalacion from tbl_factura_venta where id_factura_venta='".$id."')") or die("Error de conexion. ". pg_last_error());
}
pg_close();
 ?>