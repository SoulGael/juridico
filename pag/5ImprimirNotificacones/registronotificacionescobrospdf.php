...
for($n=0;$n<$longitud;$n++)
  {
    ...
    substring(i.direccion_instalacion from 1 for 120), c.razon_social, c.telefono, 
    c.movil_claro, c.movil_movistar, p.total ,
    l.plan, get_mes(g.fecha_periodo),extract(YEAR from g.fecha_periodo),
    (select nombre || ' ' || apellido from tbl_empleado where id_rol=8 and estado=true order by id_empleado desc limit 1) as abogado,
    (select to_char(g.num_notif,'000000')) as notificacion, i.id_instalacion, i.nombre_img
  ...
  $codigo.='</body>
    ...
    @$dompdf->render();
    $dompdf->stream("NotificacionesCobros.pdf");
